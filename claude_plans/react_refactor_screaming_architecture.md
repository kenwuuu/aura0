...
### Phase 3 — Replace imperative classes with React ⏳ IN PROGRESS (3 of 4 done)
...
- ⬜ **TooltipManager.ts → Radix Popover** (NOT STARTED — riskiest, no automated coverage). Needs `npx shadcn add popover` (not yet installed). Behavior to preserve: mouse-anchored positioning, 500ms hover-delay / 200ms hide-delay, click-to-pin, click-outside-to-close, same-card click toggle. Consumers to rewire: `MultiPlayerBoardManager` (`getTooltipManager()` + ~10 calls), `battlefieldCardActions` (6× `.hide()`), `GameResourcesDock.ts` (holds a `controlsTooltipManager`), `index.ts:164` (passes manager into dock), `CardGridItemReact.tsx`, `PileViewerReact.tsx`, `KeywordTokenGrid.tsx` (all via `useTooltipStore` in `stores/uiStore.ts`). Delete/update `TooltipManager.test.ts` (395 lines). Note: there's already a shared `tooltipManager` instance in `uiStore`, so the bridge likely lives there.
