...
- `nodeHarness.tsx` — `renderNode(Node, data, opts)` renders a single react-flow node in isolation
  (wrapped in a `ReactFlowProvider` so nodes rendering `Handle`/other react-flow primitives work).
  Prefer extracting a node's pure logic into a `*.ts` module and unit-testing that; use `renderNode`
  for the store-wiring seam only.
