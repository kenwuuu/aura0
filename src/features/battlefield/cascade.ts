 ...
 * copy #2 of the same card would repeat copy #1's spot) and playing a card
 * (`battlefieldActions`, where every play from a pile lands at the same
 * viewport centre). Both walk the same down-right diagonal, solitaire-style,
 * until they reach a free slot — so the rule and its step size live here once
 ...
 * should stay put when the spot is free (a play) pass their target position.
 */
