...
import { DeckUrlRef, parseDeckUrl, sourceLabel, upstreamApiUrl } from '../features/deck-manager/url-import/deckUrls';

/**
 * The Worker in front of Aura's static assets.
 *
 * It exists for exactly one reason: deck sites do not allow browsers to read
 * their APIs. Archidekt answers every request with a hardcoded
 * `Access-Control-Allow-Origin: http://localhost:3000` — it never reflects the
 * caller's origin — so a `fetch()` from aura0.app (or from the dev server) is
 * blocked no matter what we send. Same-origin `/api/deck-import` sidesteps CORS
 * entirely rather than trying to negotiate with it.
 *
 * Fetching here also keeps the transfer off the player's connection: an
 * Archidekt deck document is ~470KB of printings, prices and oracle text, of
 * which a decklist needs a few kilobytes. It is parsed at the edge and only the
 * card names cross the wire.
 *
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/deck-import') {
      return handleDeckImport(request, url);
    }
...
async function handleDeckImport(request: Request, url: URL): Promise<Response> {
